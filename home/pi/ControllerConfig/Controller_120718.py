    ...
    while True:
            ...
            try:
                    ...
                    if True:
                        ...
                        while True:
                            data = current_connection.recv(80)
                            ...
                            if data:
                                ...
                                if traje[:7]=='PConfig':
                                    try:
                                        ...
                                        EnvioNo='PayConfig Ok - '+ str(aaa)
                                        current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                    except:
                                        EnvioNo='Error to Pay Config'
                                        current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                if traje[len(MiMac)+len(MiIp)+2:len(MiMac)+len(MiIp)+2+3]=='300':
                                ...
                                if traje[:3]=='Num':
                                    ...
                                    if numerror==0:
                                        try:
                                                ...
                                                file.close()
                                                print('archivo num creado ok')
                                                EnvioNo='NumMaq Changed OK2 to '+str(NumMaq)
                                                current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                            #time.sleep(3.0)
                                if traje[:5]=='SSID:':
            ...
            try:
                   # Create a TCP/IP socket
                    ...
                    if data1=='999':
                            ...
                            screen.fill((0,0,0))
                            pygame.font.init
                            myfont =pygame.font.Font("super.ttf",31)
...
def luz2017(cual):
    ...
    if cual=='paseo':
        #while EnvioPuerto!='':
        #    b=1
        if TerminoOperacion==1 and punto==0 and tarjeta=='' and ctk==0:#tarjeta=='' :#punto==0 and tarjeta=='' and ctk==0: and EnvioPuerto==''
            #print("paseo")
            controlluces=0
            #%$\x99\'\xf2D\x92
...
##                #serie.write(bytes(desactivot))
            ...
            if ar>-1 and UltTar!='':
                ...
                #noviembre2017
                 ...
                 if PantallaPago==0 and tarjeta=='':#TerminoOperacion==1:# and tarjeta=='':#tarjetalista==1:# and TerminoOperacion==1:
            ...
            if len(acumulador)>0 and ctk==0: #len(a)==0 and 
                ...
                if pp>0:#and controlluces==0:                
                        ...
                        if len(mtar)>=15 and ctk==0:# and tarjeta=='':# and controlluces==0:
                            ...
                            if len(mtar)>=15:
                                acumulador=''    
            if confirmacion==True and controlluces==0 and punto==0 and ctk==0 and TerminoOperacion==1:
                controlluces=1
            if ctk==0 and punto==0:    
                MiLuz()
            #if controlluces==0:
            #    MiLuz()
        except:
...
def FinalizoTks():
    ...
    if ctk>0 and UltTar!='': 
        ...
        if True:
            ...
            while PagoOk==0:
                    try:
                        ...
                        serie.write(bytearray(bytes([0x25,0x6D,0xB6,0xDB,0x6D,0xB6,0xDF])))
                        balance=str(ctk*Multiplicador)+' Tk'
                        screen.fill((0,0,0))
                        pygame.font.init
                        myfont =pygame.font.Font("super.ttf",35)
                        bg = pygame.image.load("fondot.jpg")
        ...
        if True:#finally:
            ...
            myfont =pygame.font.Font("super.ttf",40)
            if cancelotk==0:
                label = myfont.render(LabelPago, 1, (0,180,0))
            else:
                label = myfont.render("DISABLE PAY", 1, (0,180,0))
            #label = myfont.render(que[14:30], 1, (0,255,23))
            screen.blit(label,(((infoObject.current_w - label.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)-40))
            myfont =pygame.font.Font("super.ttf",38)
...
##    EnvioPuerto=bytearray([0x30,0x01,0x35])
            ...
            controlluces=1
            if ctk==0 and tarjeta=='':#True:#int(ctk)==0:#if int(ultctk) == int(ctk):
                #ctk=0
                #ultctk=0
                time.sleep(1.2)
                #clock = pygame.time.Clock()
...
def MuestraTarjeta():
    ...
    while True:
        ...
        if len(tarjeta)==15 and YaEntre==0 and TerminoOperacion==1 and ctk==0:
...
def MiLuz():
    ...
    if True:
        b=1
            ...
            #print('entro a luces')
        if TerminoOperacion==1 and ledcual==1 and controlluces==1 and UsuarioConectado==0 and ctk==0:
                controlluces=0    
                ledcual=2
                serie.flush()
                serie.write(bytes(activot))
        ...
        elif TerminoOperacion==1 and ledcual==2 and controlluces==1 and UsuarioConectado==0 and ctk==0:
                controlluces=0
                ledcual=1
                serie.flush()
                serie.write(bytes(activot))
...
#Expandir File System.------------------------
        ...
        esto='Por favor espere...'
        label = myfont.render("Expansion de Memoria", 1, (0,255,20))
...
#---------------
        ...
        while IPFINAL=='':
              ...
              if esto!='':
                      ...
                      #MiMac1=MiMac1[0:2]+MiMac1[3:5]+MiMac1[6:8]+MiMac1[9:11]+MiMac1[12:14]+MiMac1[15:17]
                      #print(MiMac1)
                      esto='SIN CONEXION CON SERVIDOR'
                      label3 = myfont2.render("Verificar Wifi o Servidor Encendido", 1, (0,255,255))
              label2 = myfont1.render(esto, 1, (255,0,0))
              screen.blit(label2,(((infoObject.current_w - label2.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)+100))
...
if LOCAL=='0' and EMPRESA=='0':
    saveQRCodePNG('qr',4,NumMaq+'@'+IPFINAL) #genero QR con numero de maquina
else:
    saveQRCodePNG('qr',4,NumMaq+LOCAL+'#'+EMPRESA+'@'+IPFINAL) #genero QR con numero de maquina
PantFull()
#programafinal-----------------------------
