...
#LEDS MODOS -EZEQUIEL CICCOTELLI - 02/2019
#modo1 -> VRA
#modo2 -> RVA
#modo3 -> ARV
...
global LabelRecarga1 #1021
global LabelRecarga2 #1022
...
LabelRecarga1='Insert Coins and'
LabelRecarga2='Swipe or Tap your Card'
#------------------------
PN='-'
PV='-'
...
def MantengoCone():
    global tarjeta
    print('entro mantengocone')
    #cuando=''
    #ahora=''
    #Cuando=datetime.now() + timedelta(seconds=5)
    if tarjeta=='':
        #Ahora=datetime.now()
        #if Ahora>=Cuando and tarjeta=='':
            try:
                print('previo a conectar')
                sockm = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                server_address = (IP,5000)#192.168.2.50 -- IAAPA
                print('connecting to %s port %s' % server_address)
                sockm.settimeout(1.0)
                sockm.connect((str(IP),5000))#(server_address)
                try:
                    sockm.send(NumMaq.encode())
                finally:
                    sockm.close()
                    print('cn - Ok')
                Cuando=datetime.now() + timedelta(seconds=300)
            except:
                print('cn - error')
            #    Cuando=datetime.now() + timedelta(seconds=1)
...
def saveQRCodePNG(Nombre,Tam,Que):
        try:
            ...
            #messagebox.showinfo('Continue',Que)
...
def listen():      
    ...
    RECV_BUFFER = 150 # Advisable to keep it as an exponent of 2
    PORT = 5001
                    ...
                    if True:
                        ...
                        if True:
                            ...
                            if data:
                                ...
                                print(traje)
                                EnvioNo=''
                                if traje[:9]=='ledmode:1':                                    
                                    ModoLed=1
                                    
                                    file=open('/home/pi/ControllerConfig/ledmode.mc','w')
                                    file.write('1')
                                    file.flush()
                                    file.close()
                                    time.sleep(1.0)
                                    EnvioNo='OK '+str(ModoLed)
                                    current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                if traje[:9]=='ledmode:2':
                                    ModoLed=2
                                ...
                                if traje[:3]=='lg22:':
                                    ...
                                    if int(NumMaq,16) > int('000F',16):
                                        try:
                                            ...
                                            if PN!="-":
                                                ...
                                                if  int(NumMaq,16) > int('000F',16) and TipoDebito!='4':
                                                    saveQRCodePNG('qr',4,CRCID+NumMaq+LOCAL+'#'+EMPRESA+'@'+IPFINAL+'*'+ trajepago) #genero QR con numero de maquina
                                                else:
                                                    saveQRCodePNG('qr',2,CRCID+NumMaq+LOCAL+'#'+EMPRESA+'@'+IPFINAL+'*'+ trajepago) #genero QR con numero de maquina
                                                PantFull()
                                            confirmacion=False
                                    ...
                                    if numerror==0:
                                        ...
                                        #        current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                            ...
                                            try:
                                                    ...
                                                    file.write('static routers=10.0.0.1'+'\n')
                                                    file.write('static domain_name_servers=10.0.0.1'+'\n')
                                                    file.write('static domain_search=10.0.0.1')
                                                    file.write('\n')
                                                    file.write('\n')
                                                    file.write('SSID '+str(WIFIN)+'\n')
                                                    #file.write('interface wlan0'+'\n')
                                    ...
                                    msj=subprocess.check_output("iwconfig",shell=True).decode('utf8')
                                    current_connection.send(msj.encode(encoding='UTF-8',errors='ignore'))
                                elif traje[:7]=='pcolor1':
                                    ColorLed('33','010','010','010','010','010','010','010','010','010','010','010','010','010','010','010','010')
                                    msj='OK'#subprocess.check_output("iwconfig",shell=True).decode('utf8')
                                    current_connection.send(msj.encode(encoding='UTF-8',errors='ignore'))
                                elif traje[:7]=='pcolor2':
                                    ColorLed('33','100','100','100','100','100','100','100','100','100','100','100','100','100','100','010','100')
                                    msj='OK'#subprocess.check_output("iwconfig",shell=True).decode('utf8')
                                    current_connection.send(msj.encode(encoding='UTF-8',errors='ignore'))
                                elif traje[:7]=='pcolor3':
                                    ColorLed('33','001','001','001','001','001','001','001','001','001','001','001','001','001','001','001','001')
                                    msj='OK'#subprocess.check_output("iwconfig",shell=True).decode('utf8')
                                    current_connection.send(msj.encode(encoding='UTF-8',errors='ignore'))
                                elif traje[:8]=='pcolor21':
                                    ColorLed('34','010','010','010','010','010','010','010','010','010','010','010','010','010','010','010','010')
                                    msj='OK'#subprocess.check_output("iwconfig",shell=True).decode('utf8')
                                    current_connection.send(msj.encode(encoding='UTF-8',errors='ignore'))
                                elif traje[:8]=='pcolor22':
                                    ColorLed('34','100','100','100','100','100','100','100','100','100','100','100','100','100','100','010','100')
                                    msj='OK'#subprocess.check_output("iwconfig",shell=True).decode('utf8')
                                    current_connection.send(msj.encode(encoding='UTF-8',errors='ignore'))
                                elif traje[:8]=='pcolor23':
                                    ColorLed('34','001','001','001','001','001','001','001','001','001','001','001','001','001','001','001','001')
                                    msj='OK'#subprocess.check_output("iwconfig",shell=True).decode('utf8')
                                    current_connection.send(msj.encode(encoding='UTF-8',errors='ignore'))    
                                    ...
                                    current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                    EnvioNo='Controller: Last Receive: '+ str(LeiTcp) + '\r\n'
                                    current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                    try:
                                        EnvioNo='Controller: My LedMode is '+str(LedMode)+ '\r\n'
                                        current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                    except:
                                        b=1
                                    ...
                                    current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                    EnvioNo='Controller: Version is '+str(SVersion)+ '\r\n'
                                    current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                ...
                                #-----------------------
                                      ...
                                      PULSO=traje[6:7]
                                      print(PULSO)
                                      vpago=bytearray()#[0x29,0x10])#,0x00,0x00,0x00,0x00,0x00])#pago rapido-coin rapido
                                      vpago.append(0x29)
                                      if int(PULSO)<4:
                                          vpago.append(0x02)
                                      if int(PULSO)>=4 and int(PULSO) <7:
                                          vpago.append(0x00)
                                      if int(PULSO)>6:
                                          vpago.append(0x01)
                                      #print(PULSO)
                                ...
                                #-----------------------
                                ...
                                elif traje[0:17]==MiMac and traje.find('11A')>0:
                                  time.sleep(0.2)
                                  EnvioNo='103'
                                  current_connection.send(EnvioNo.encode(encoding='UTF-8',errors='ignore'))
                                  serie.flush()
                                  ...
                                  if NumMaq==traje[(21+sumar):(25+sumar)]:
                                      ...
                                      print(str(Maximo))
                                      TipoDebito=str(traje[(54+sumar):(55+sumar)])
                                      TipoPago=str(traje[(55+sumar):(56+sumar)])
                                      CRCID=str(traje[(56+sumar):(61+sumar)])
                                      CRCID=CRCID.strip()
                                      VPAY=str(traje[(61+sumar):(62+sumar)])
                                      VPAY=VPAY.strip()
                                      time.sleep(0.050)
                                      time.sleep(2.0)
                                      vpago=bytearray()#[0x29,0x10])#,0x00,0x00,0x00,0x00,0x00])#pago rapido-coin rapido
        ...
        if True:#while treintento<11 and len(data)<46: #reintenta si no recibe respuesta del tcpserver
            try:
                   # Create a TCP/IP socket
                    while len(data)<3 and reintento<= 30 and buscartarjeta==1:
                        try:
                            time.sleep(0.005)
                            ConectoMaster=0
                            print('previo a conectar')
                            ...
                            print('connecting to %s port %s' % server_address)
                            if treintento==0:
                                sockd.settimeout(1.5)
                            else:
                                sockd.settimeout(0.8)
                            #try:
                            sockd.connect((str(IP),5000))
                            #except:
                                #screen.fill((0,0,0))
                                #pygame.font.init
                                ...
                                #label = myfont.render('NO CONECTO', 1, (0,255,20))
                            ...
                            sockd.settimeout(3.0)
                            message=str(message)
                            sockd.send(message.encode())#.encode(encoding='UTF-8',errors='ignore'))
                            ConectoMaster=1
                            ...
                            #    treintento=2
                            #    hex('hola')
                            if treintento>=19 and ConectoMaster==0:
                                ...
                                pygame.font.init
                                myfont =pygame.font.Font("super.ttf",35)
                                myfont1 =pygame.font.Font("super.ttf",15)

                                bg = pygame.image.load("fondot.jpg")
                                screen.blit(bg,(0,0))

                                esto=str(e)#balance#data#str(tarjeta)+str(' Ok')
                                label = myfont.render(LabelError, 1, (0,255,20))
                                screen.blit(label,(((infoObject.current_w - label.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)-40))
                                myfont =pygame.font.Font("super.ttf",16)
                                label1 = myfont1.render(esto, 1, (0,255,20))
                                screen.blit(label1,(((infoObject.current_w - label1.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)+40))
                                pygame.display.flip() 
                                time.sleep(1.0)
                                b=1
                    ...
                    if data1=='999':
                            ...
                            if len(data1)>46:
                                ...
                                mok='103'
                                mok=str(mok)
                                sockd.sendall(mok.encode(encoding='UTF-8',errors='ignore'))
                                ...
                                controlluces=0
                                t1=0
                                ColorLed('25','010','010','010','111','010','010','010','010','010','010','010','010','010','010','111','010')
                ...
                if treintento==2:
                    ...
                    esto=str(e)#str(tarjeta)#'+str(' Ok')
                    label = myfont.render(LabelError, 1, (0,255,20))
                    screen.blit(label,(((infoObject.current_w - label.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)-40))
                    myfont =pygame.font.Font("super.ttf",15)
                    label1 = myfont.render(esto, 1, (0,255,20))
                    screen.blit(label1,(((infoObject.current_w - label1.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)+40))
                    pygame.display.flip() 
                    time.sleep(1)
...
def TiempoTk():
    ...
    EntrarTimer=False
    TimeOut=False
    Cuando=datetime.now() + timedelta(seconds=60)
    while True:
        time.sleep(0.010)
        Ahora1=datetime.now()
        #if int(ctk)>0:
        ...
        if PagandoTk==False and int(ctk)>0:
            ...
            if Ahora >= HTimeOut and TimeOut==False:
                ...
                if PagandoTk==False and int(ctk)>0 and PantallaPago==0:
...
vpago=bytearray()#[0x29,0x10])#,0x00,0x00,0x00,0x00,0x00])#pago rapido-coin rapido
vpago.append(0x29)
vpago.append(0x00) #2 byte es velocidad 10 lento, 22 rapido
serie.write(bytes(vpago))
...
def ColorLed(p,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13,l14,l15,l16):
    global ModoLed

    #a=""

    #inversion de colores verde y rojo 2018
    if ModoLed==2:
        l1=l1[2:3]+l1[:1]+l1[1:2]
        l2=l2[2:3]+l2[:1]+l2[1:2]
        l3=l3[2:3]+l3[:1]+l3[1:2]
        l4=l4[2:3]+l4[:1]+l4[1:2]
        l5=l5[2:3]+l5[:1]+l5[1:2]
        l6=l6[2:3]+l6[:1]+l6[1:2]
        l7=l7[2:3]+l7[:1]+l7[1:2]
        l8=l8[2:3]+l8[:1]+l8[1:2]
        l9=l9[2:3]+l9[:1]+l9[1:2]
        l10=l10[2:3]+l10[:1]+l10[1:2]
        l11=l11[2:3]+l11[:1]+l11[1:2]
        l12=l12[2:3]+l12[:1]+l12[1:2]
        l13=l13[2:3]+l13[:1]+l13[1:2]
        l14=l14[2:3]+l14[:1]+l14[1:2]
        l15=l15[2:3]+l15[:1]+l15[1:2]
        l16=l16[2:3]+l16[:1]+l16[1:2]
    if ModoLed==3:
        l1=l1[2:3]+l1[1:2]+l1[:1]
        l2=l2[2:3]+l2[1:2]+l2[:1]
        l3=l3[2:3]+l3[1:2]+l3[:1]
        l4=l4[2:3]+l4[1:2]+l4[:1]
        l5=l5[2:3]+l5[1:2]+l5[:1]
        l6=l6[2:3]+l6[1:2]+l6[:1]
        l7=l7[2:3]+l7[1:2]+l7[:1]
        l8=l8[2:3]+l8[1:2]+l8[:1]
        l9=l9[2:3]+l9[1:2]+l9[:1]
        l10=l10[2:3]+l10[1:2]+l10[:1]
        l11=l11[2:3]+l11[1:2]+l11[:1]
        l12=l12[2:3]+l12[1:2]+l12[:1]
        l13=l13[2:3]+l13[1:2]+l13[:1]
        l14=l14[2:3]+l14[1:2]+l14[:1]
        l15=l15[2:3]+l15[1:2]+l15[:1]
        l16=l16[2:3]+l16[1:2]+l16[:1]
    ...
    color=l1+l2+l3+l4+l5+l6+l7+l8+l9+l10+l11+l12+l13+l14+l15+l16
    envio=a24+color
    a=hex(int(envio,2))
...
def colorfijo2(p,c1,c2,c3,c4):
    ...
    #inversion de colores verde y rojo 2018
    if ModoLed==2:
        c1=c1[2:3]+c1[:1]+c1[1:2]
        c2=c2[2:3]+c2[:1]+c2[1:2]
        c3=c3[2:3]+c3[:1]+c3[1:2]
        c4=c4[2:3]+c4[:1]+c4[1:2]
    if ModoLed==3: #l1[1:2]+l1[2:3]+l1[:1]
        #l1[2:3]+l1[1:2]+l1[:1]
        c1=c1[2:3]+c1[1:2]+c1[:1]
        c2=c2[2:3]+c2[1:2]+c2[:1]
        c3=c3[2:3]+c3[1:2]+c3[:1]
        c4=c4[2:3]+c4[1:2]+c4[:1]
...
##                punto=1
            ...
            if ar>-1 and UltTar!='':
                ...
                #noviembre2017
                 ...
                 #ColorLed('24','001','001','001','001','001','001','001','001','001','001','001','001','001','001','001','001')
                 #if confirmacion==True:
                 ...
                 #-----PARA PAGO FISICO---
            ar=recibi.find('A',0)
            if ar>-1 and UltTar!='' or ar>-1 and  TipoDebito=='4' and TipoPago=='3':
                 print('Entro Fisico')
                 if PagandoTk==False:
                     #if confirmacion==True:
                     acumulador=''
...
##                     ultctk=ultctk+(ctk*Multiplicador)
                 ...
                 if TipoDebito=='4' and TipoPago=='3':
                     HTimeOut=datetime.now()+timedelta(seconds=5)
                 else:
                     HTimeOut=datetime.now()+timedelta(seconds=4)
                 #HTimeOut=datetime.now()+timedelta(seconds=4)
                 if PantallaPago==0 and TerminoOperacion==1:# and tarjetalista==1:
                     ...
                     if TipoDebito=='4' and TipoPago=='3':
                         ...
                         esto= ctk
                     esto1=str(esto)
                     label1 = myfont.render(esto1, 1, (0,255,20))
                     screen.blit(label1,(((infoObject.current_w - label1.get_width()) / 2), ((infoObject.current_h - label1.get_height()) / 2)+55))#(label, (((infoObject.current_w - label.get_width()) / 2),160 ))
                     pygame.display.flip()
                 #colorfijo2('24','100','000','100','000')   
                        #luz=1
                ...
                #noviembre2017
                 ...
                 ###print(ctk)
            recibi=''
            if len(acumulador)>0 and ctk==0 or TipoDebito=='4': #len(a)==0 and 
                ...
                if pp>0:#and controlluces==0:                
                        ...
                        #while principal==0:
                        #    time.sleep(0.010)
                        if len(mtar)>=15 and TipoDebito=='4' and ctk>0:
                            UltTar=mtar
                            HTimeOut=datetime.now()
                        if len(mtar)>=15 and ctk==0 and tarjeta=="" :# and tarjeta=='':# and controlluces==0:
                            if TipoDebito!='4':
                                tarjeta=mtar
                            #TerminoOperacion=0
                            ...
                            if len(mtar)>=15 and TipoDebito!=4:
                                acumulador=''
                                punto=0
...
def FinalizoTks():
    ...
    if ctk>0:#: and UltTar!='': 
        ...
        while UltTar=='':
            ...
            serie.write(bytes(activot))
            time.sleep(0.250)
            ColorLed('25','101','101','000','000','000','101','000','000','000','000','000','000','101','000','000','000')
            screen.fill((0,0,0))
            bg = pygame.image.load("fondot.jpg")
            screen.blit(bg,(0,0))                         
            ...
            screen.blit(label,(((infoObject.current_w - label.get_width()) / 2),120))#(label, (((infoObject.current_w - label.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)))
            #label1 = myfont.render(LabelRecarga2, 1, (255,255,255))
            #screen.blit(label1,(((infoObject.current_w - label1.get_width()) / 2),170))

            #myfont = pygame.font.SysFont("monospace", 45)
            
            label3 = myfont2.render('www.magneticash.com', 1, (255,255,255))
            screen.blit(label3,(135,305))
            label4 = myfont3.render('SN:'+str(IPFINAL[3:]), 1, (0,255,255))
            screen.blit(label4,(415,310))
            
            #if EMPRESA!='0':
            #    QrImg = pygame.image.load('qr.png')
            #    screen.blit(QrImg, (0,239))
            #else:
            #    QrImg = pygame.image.load('qr.png')
            #    screen.blit(QrImg, (0,239))
            pygame.display.flip()
        ...
        if True:
            ...
            while PagoOk==0:
                    try:
                        ...
                        if tkr==0:
                            sockp.settimeout(1.5)
                        else:
                            sockp.settimeout(0.75)
                        sockp.connect((str(IP),5000))#(server_address)
                        ...
                        sockp.sendall(message.encode(encoding='UTF-8',errors='ignore'))
                        ...
                        if Maximo!=0 and (ctk*Multiplicador)<=Maximo:
                            ...
                            ultctk1=ultctk+(ctk*Multiplicador)
                        serie.write(bytearray(bytes([0x24,0x6D,0xB6,0xDB,0x6D,0xB6,0xDF])))
        ...
        if True:#finally:
            ...
            que1=''
            if TipoDebito=='4' and TipoPago=='3':
                traje2=data.decode('utf8')
                LabelPago=traje2[14:30]
                que1=traje2[31:46]
            ...
            if TipoDebito!='4' and TipoPago != '3':
                ...
                label1 = myfont.render(str(que1), 1, (0,255,20))
            if cancelotk==0:# and TipoDebito!='4':
                screen.blit(label1,(((infoObject.current_w - label1.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)+40))
            pygame.display.flip() 
            #pygame.display.update()
...
##    EnvioPuerto=bytearray([0x30,0x01,0x35])
            ...
            controlluces=1
            if TipoDebito=='4' and TipoPago=='3' and ctk==0:
                UltTar='' #para que limpie tarjeta de pago
            if ctk==0:#True:#int(ctk)==0:#if int(ultctk) == int(ctk):
                #ctk=0
                #ultctk=0
                punto=0
...
def PantFull():
                    ...
                    #try:
                    #    os.system('sudo omxplayer silencio.wav')
                    #except:
                    #    b=1
                        ...
                        if int(ctk)==0:
                            if TipoDebito=='4' and TipoPago=='3':
                                ...
                                myfont3 =pygame.font.Font("super.ttf",10)
                                    ...
                                    # render text
                                label = myfont.render(LabelRecarga1, 1, (255,255,255))
                                screen.blit(label,(((infoObject.current_w - label.get_width()) / 2),120))#(label, (((infoObject.current_w - label.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)))
                                label1 = myfont.render(LabelRecarga2, 1, (255,255,255))
                                screen.blit(label1,(((infoObject.current_w - label1.get_width()) / 2),170))
                                ...
                                msj=str(msj)
                                print(msj)
                                myfont4=pygame.font.SysFont("comicsansms",15)
                                if msj.find('Quality=')>0:
                                    rdesde=msj.find('Quality=')+8
                                    rhacia=msj.find('Quality=')+10
                                    print(rdesde)
                                    print(rhacia)
                                    sred=msj[rdesde:rhacia]
                                    print('senal: '+ sred)
                                    if int(sred)<30:
                                        sred=0
                                    else:
                                        sred=int(sred)-30
                                    try:
                                        sred=((int(sred)/40)*100)
                                    except:
                                        sred=0
                                    sred=round(sred,0)
                                    if int(sred)>=0:
                                        label6 = myfont4.render(str(sred)+'%', 1, (255,0,0))
                                    if int(sred)>12:
                                        label6 = myfont4.render(str(sred)+'%', 1, (255,165,0))
                                    if int(sred)>40:
                                        label6 = myfont4.render(str(sred)+'%', 1, (0,255,0))                                
                                    screen.blit(label6,(1,2))
                                if EMPRESA!='0':
                                    QrImg = pygame.image.load('qr.png')
                                    screen.blit(QrImg, (40,116))
                                else:
...
##            Debitar()
            #tarjeta=''
        time.sleep(0.010)
        if len(tarjeta)==15 and TipoDebito=='4':
            UltTar=tarjeta
        if len(tarjeta)==15 and  ctk==0 and YaEntre==0 and principal==1:# or len(tarjeta)==15 and  TipoDebito=='4' and YaEntre==0 and principal==1: #and principal==1: #and YaEntre==0:# and TerminoOperacion==1 and
            YaEntre=1
            #while principal==0:
            #    time.sleep(0.010)
            ...
            Debitar()
            YaEntre=0
            #while esto1==False and ReintentoDebito < 15:
            #    esto1=Debitar()# is False:
            #    print('debitar salio con ' + str(esto1) + ' Reintentos: '+ str(ReintentoDebito))
            #    ReintentoDebito=ReintentoDebito+1
            #if ReintentoDebito>=15:
            #    YaEntre=0
            #    TerminoOperacion=1
            #    principal=1
            #    os.system('sudo ifconfig wlan0 down')  
        ...
        try:
            ...
            if Reinicio==1:
                try:
                    ...
                    serie.write(pcorto)
                    Reinicio=0
                    print('Envio Conf de Pago')
                except Exception as err:
                    print('error en B '+err)
                    serie.flush()
                    serie.write(pcorto)
                    serie.flush()
                    
            if principal==1 and tarjeta=='' and doblev=='' and PantallaPago==0:
                serie.flush()
                serie.write(bytes(activot))
                time.sleep(0.100)
            if doblev=='W' and TipoDebito=='4':
                doblev=''
            if doblev=='W' and UsuarioConectado==0 and principal==1 and tarjeta=='' and PantallaPago==0:
                        controlluces=0
                        #time.sleep(1.0)
                        #while controlluces==0:
                        serie.flush()
                            #serie.write(bytes(bytearray([0x25,0x49,0x00,0x00,0x49,0x00,0x00])))
                        colorfijo2('25','000','000','000','010')
                        time.sleep(0.050)
                        doblev='W'
                        #ConfirmacionTarjeta=False
...
ColorLed('34','100','100','100','100','100','100','100','100','100','100','100','100','100','100','100','100')

time.sleep(1.0)
#primer color
ColorLed('33','101','101','101','101','101','101','101','101','101','101','101','101','101','101','101','101')
time.sleep(1.0)
...
#ModoLed
try:
    file=open('/home/pi/ControllerConfig/ledmode.mc','r')
    ModoLed=int(file.readline())
    file.close
except:
    ModoLed=2
...
#---------------
        ...
        msj=subprocess.check_output("cat /sys/class/net/wlan0/address",shell=True).decode('utf8')
        MiMacF=msj.strip()
        file=open("/home/pi/ControllerConfig/mmc.mc","w")
        file.write(str(MiMacF))
        file.flush()
        file.close()
...
#-------------------
#verifica si existe mac
msj=subprocess.check_output("cat /sys/class/net/wlan0/address",shell=True).decode('utf8')
MiMacF=msj.strip()
#ModoLed
try:
    file=open('/home/pi/ControllerConfig/mmc.mc','r')
    MiMac=file.readline()
    file.close
except:
    MiMac='00:00:00:00:00:00'
...
if MiMacF!=MiMac:
    while 1:
        screen.fill((0,0,0))
        pygame.font.init
        myfont =pygame.font.Font("super.ttf",33)
        myfont1 =pygame.font.SysFont("comicsansms",30)
        myfont2 =pygame.font.SysFont("comicsansms",30)
        ...
        esto='Blocked'
        label = myfont.render("MAGNETIC CASH SYSTEM", 1, (255,255,255))
        screen.blit(label,(((infoObject.current_w - label.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)-40))
        myfont =pygame.font.Font("super.ttf",50)
        label1 = myfont.render(esto, 1, (255,75,0))
        screen.blit(label1,(((infoObject.current_w - label1.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)+40))                          
        pygame.display.flip()
        colorfijo2('28','010','000','000','000')
        time.sleep(0.8)
...
#LO QUITE DESPUES DE ARREGLAR WIFI AGOSTO 2018
#if estahotspot==0:        
#    os.system('sudo ifconfig wlan0 down')
#time.sleep(0.25)
#    os.system('sudo ifconfig wlan0 up')
...
while PN=='-':
    try:
      if Conecto==0 and contadorwifi>3:  
          time.sleep(0.010)
          screen.fill((0,0,0))
          bg = pygame.image.load("fondot.jpg")
          screen.blit(bg,(0,0))
          esto='Wifi Connection TimeOut'
          myfont =pygame.font.Font("super.ttf",24)
          label = myfont.render("CONNECTING TO SYSTEM", 1, (0,255,255))
          screen.blit(label,(((infoObject.current_w - label.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)-40))
          myfont =pygame.font.Font("super.ttf",21)
          label1 = myfont.render(esto, 1, (120,255,0))
          screen.blit(label1,(((infoObject.current_w - label1.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)+20))
          porcentaje=contadorwifi / 390
          #print(porcentaje)
          porcentaje=porcentaje*100
          #print(porcentaje)
          aa=str(porcentaje)
          print(aa[:4])
          label2 = myfont.render(aa[:4] +' %', 1, (120,255,0))
          screen.blit(label2,(((infoObject.current_w - label2.get_width()) / 2), ((infoObject.current_h - label2.get_height()) / 2)+60))
          pygame.display.flip()
          ColorLed('28','000','000','010','001','010','000','000','000','010','001','000','000','000','010','001','010')
      if True:#Conecto==0:
          va=va-1  
          if va<=0:
              os.system('sudo ifconfig wlan0 down')
      ...
      if Conecto==1:
              ...
              if primeravez==0:# and UsuarioConectado==0:#cambiednsh==1 and UsuarioConectado==0 or primeravez==0 and UsuarioConectado==0:#Conecto==0:
                      primeravez=1
                      ...
                      os.system('wpa_cli -i wlan0 reconfigure')
                      time.sleep(3.0)
      elif contadorwifi==390 and Conecto==0:
              screen.fill((0,0,0))
              pygame.font.init
              myfont =pygame.font.Font("super.ttf",30)
              myfont1 =pygame.font.SysFont("comicsansms",30)
              ...
              contadorwifi=0
              if cambiednsh==0:# and UsuarioConectado==0:# and contadorwifi==10:#Conecto==0:
      ...
      if Conecto==1:
          #time.sleep(2.0)
          print('entro a conecto 1 y abre socket')
          sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
          server_address = (IP,5000)#192.168.2.50 -- IAAPA
          #print('connecting to %s port %s' % server_address)
          sock.settimeout(1.5)
          sock.connect((str(IP),5000))#(server_address)
          tcpserver=1
          ...
          if Idioma==0:
                  ...
                  try:
                      ...
                      b=1
                  try:
                      LabelRecarga1=mijson["1021"]
                      LabelRecarga2=mijson["1022"]
                  except:
                      b=1
                  Idioma=1
                  message='103'
                  ...
                  if data[sumar:sumar+3]=='999':
                      ...
                      esto=LabelNR
                      label2 = myfont.render(esto, 1, (255,255,255))
                      screen.blit(label2,(((infoObject.current_w - label2.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)-25))
                      msj=subprocess.check_output("cat /sys/class/net/wlan0/address",shell=True).decode('utf8')
                      MiMac1=msj.strip()
                      MiMac1=MiMac1[0:2]+MiMac1[3:5]+MiMac1[6:8]+MiMac1[9:11]+MiMac1[12:14]+MiMac1[15:17]
                      label4 = myfont1.render(LabelReemplazo, 1, (255,255,255))
                      screen.blit(label4,(((infoObject.current_w - label4.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)))
                      label5 = myfont1.render('replace:1', 1, (255,255,255))
                      screen.blit(label5,(((infoObject.current_w - label5.get_width()) / 2), ((infoObject.current_h - label.get_height()) / 2)+20))
                      myfont3 =pygame.font.Font("super.ttf",10)
                      label4 = myfont3.render('SN:'+str(IPFINAL[3:]), 1, (0,255,255))
                      screen.blit(label4,(415,310))
                      #saveQRCodePNG('qrip',2,str(IPFINAL))
                      saveQRCodePNG('qrip',3,CRCID+NumMaq+LOCAL+'#'+EMPRESA+'@'+IPFINAL+'*'+trajepago) #genero QR con numero de maquina
                      QrImg = pygame.image.load('qrip.png')
                      screen.blit(QrImg, (180,200))
                      pygame.display.flip()
                  if len(data)>30:    
                      ...
                      Maximo=int(float(Maximo.strip()))
                      TipoDebito=str(data[(36+sumar):(37+sumar)])
                      TipoPago=str(data[(37+sumar):(38+sumar)])
                      CRCID=str(data[(38+sumar):(43+sumar)])
                      CRCID=CRCID.strip()
                      VPAY=str(data[(43+sumar):(44+sumar)])
                      VPAY=VPAY.strip()
          ...
          if str(err)=="invalid literal for int() with base 10: ''":
              ...
              label4 = myfont3.render('SN:'+str(IPFINAL[3:]), 1, (0,255,255))
              screen.blit(label4,(415,310))
              saveQRCodePNG('qrip',3,CRCID+NumMaq+LOCAL+'#'+EMPRESA+'@'+IPFINAL+'*'+trajepago)#str(NumMaq)+'@'+str(IPFINAL))
              #saveQRCodePNG('qr',2,CRCID+NumMaq+LOCAL+'#'+EMPRESA+'@'+IPFINAL+'*'+trajepago) #genero QR con numero de maquina
              QrImg = pygame.image.load('qrip.png')
              screen.blit(QrImg, (180,200))
              pygame.display.flip()
...
if Conecto==1:
    ...
    #luces
    #miroconexion=threading.Thread(target=MantengoCone())#MiLuz #Luz2018-giro Manual
    #miroconexion.start()
