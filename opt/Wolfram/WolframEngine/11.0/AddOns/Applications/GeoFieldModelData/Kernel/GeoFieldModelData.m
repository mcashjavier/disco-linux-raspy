...
$ivproperties={All,Min,Max,Mean,StandardDeviation,Interval};
...
geoElevation[{longitude1_,longitude2_},{latitude1_,latitude2_},zoom_,type_]:=Module[
	{z=zoom,rows,columns,points,lat1=Min[latitude1,latitude2],lat2=Max[latitude1,latitude2],
		long1=Min[longitude1,longitude2],long2=Max[longitude1,longitude2],array},
	array={Sort@{lat1,lat2},Sort@{long1,long2}};
	If[MatchQ[zoom,Automatic],
		z=Min[GIS`EstimateZoom[array,Automatic,Automatic,{Automatic},False,1]];
		If[IntegerQ[z],
	...
	If[MatchQ[points,GeoPosition[List[_List..]]],
		points[[1]],
		Message[type::elev];
...
getTime[Automatic]:=Module[{res = now}, 
 	If[DateObjectQ[res], 
 		res, 
  		Message[System`GeomagneticModelData::now]; $Failed
  	]
];
getTime[x_]:=Module[{res = DateObject[x]}, 
 	If[DateObjectQ[res], 
 		res, 
...
geoComponent[{vert_?NumericQ,north_?NumericQ,east_?NumericQ},"Declination"]:=If[Abs[north]<$MachineEpsilon,Indeterminate,
	ArcTan[east/north]*180/Pi];
geoComponent[{vert_?NumericQ,north_?NumericQ,east_?NumericQ},"Inclination"]:=If[Norm[{north,east}]<$MachineEpsilon,Indeterminate,
	ArcTan[vert/Norm[{north,east}]]*180/Pi];
...
iGeoModelDataAssociation[loc_,comp_,iv_,func_,opt:OptionsPattern[]]:=Module[{date,location},
	date=If[KeyExistsQ[loc,"Date"],loc["Date"],now];
...
PList[n_, theta_] := 2*Sqrt[2 Pi]*SpecialFunctions`SphericalHarmonicYTriangularArray[n, N[theta-Pi/2]];
...
calculateComponent["Array",{r_,th_,lam_},lat_,opts:OptionsPattern[]]:=Module[{result=iGeogravityField[{r,th,lam}],potential,
	...
	potential=result[[-1]]+If[rotQ,-1/2omegaEarth^2 a^2 Cos[th]^2,0];
	result=GeocentricComponentsToGeodeticComponents[result[[;;3]],lat,th];
	result={-1,-1,-1}*result[[{1,3,2}]]+If[rotQ,{-omegaEarth^2 a Cos[th]^2, -omegaEarth^2 a Cos[th] Sin[th],0},{0,0,0}];
	{result,potential}
]
calculateComponent[All,{r_,th_,lam_},lat_,opts:OptionsPattern[]]:=Module[{result=iGeogravityField[{r,th,lam}],potential,
		rotQ=OptionValue["Rotation"],a=r},
	potential=Quantity[result[[-1]]+If[rotQ,-1/2omegaEarth^2 a^2 Cos[th]^2,0],"Joules"/"Kilograms"];
	result=GeocentricComponentsToGeodeticComponents[result[[;;3]],lat,th];
	result={-1,-1,-1}*result[[{1,3,2}]]+If[rotQ,{-omegaEarth^2 a Cos[th]^2, -omegaEarth^2 a Cos[th] Sin[th],0},{0,0,0}];
...
calculateComponent["Potential",{r_,th_,lam_},lat_,opts:OptionsPattern[]]:=Module[{pot=VEGM[{r,th,lam}],
	a=r,rotQ=OptionValue["Rotation"]},
	geoComponent[pot+If[rotQ,-1/2 omegaEarth^2 a^2 Cos[th]^2,0],"Potential"]
];
calculateComponent[comp_,{r_,th_,lam_},lat_,opts:OptionsPattern[]]:=Module[
	...
	result=GeocentricComponentsToGeodeticComponents[result,lat,th];
	result={-1,-1,-1}*result[[{1,3,2}]]+If[rotQ,{-omegaEarth^2 a Cos[th]^2, -omegaEarth^2 a Cos[th] Sin[th],0},{0,0,0}];
...
iGeogravityModelData[loc_,comp_,iv_,opts:OptionsPattern[]]:=Module[{location,userzQ,tempht,
	...
	If[MatchQ[location,{_,_,_}],
		If[Not[NumericQ[location[[-1]]]||TrueQ[location[[-1]]]],Message[System`GeogravityModelData::elev];location[[-1]]=0]];
...
Options[iGeogravityModelCompute]:={includeRotation->True,GeoZoomLevel->Automatic, modelDegree->60};
...
iGeogravityModelCompute[{{lat1_,long1_},{lat2_,long2_},elevation_},comp_?(StringQ[#]||MatchQ[#,All]&),opts:OptionsPattern[]]:=Module[
	{lam,th,r,inputs,lat,L,PGravityAll,elevationmeters=elevation,last,result,potential,
		o=OptionValue[modelDegree],spin=OptionValue[includeRotation],a=EarthRadius*1000,
	...
	If[TrueQ[elevation],
		...
		If[MatchQ[zoom,Automatic],zoom=GIS`EstimateZoom[{Sort@{lat1, lat2},Sort@{long1, long2}}, Automatic, Automatic, {Automatic}, False, 1]];
...
iGeogravityModelCompute[{latitudedegrees_,longitudedegrees_,elevationmeters_},comp_?(StringQ[#]||MatchQ[#,All]&),opts:OptionsPattern[]]:=Module[
	{inputs=GeodeticToGeocentric[{elevationmeters,latitudedegrees,((*180+*)longitudedegrees),0}],r,lam,th,last,
		o=OptionValue[modelDegree],spin=OptionValue[includeRotation],a=EarthRadius*1000,
		L,PGravityAll,result},
	last=sumlist[[o+1]]+o;
	r=inputs[[1]];th=inputs[[2]];lam=inputs[[3]];
	If[o===360,
		Cnlist=CnList;Snlist=SnList;mlist=mList;mnlist=mnList,
		Cnlist=CnList[[;;last-3]];Snlist=SnList[[;;last-3]];mlist=mList[[;;last-3]];mnlist=mnList[[;;last-3]]
	];
	...
	PGravity=PGravityAll[[4;;last]];
	PGravity2=Flatten[Table[PGravityAll[[sumlist[[n+2]];;sumlist[[n+2]]+n]],{n,2,o}]];
	result=calculateComponent[comp,{r,th,lam},latitudedegrees Degree,"Rotation"->spin];
	Which[comp===All,
		result,
		comp==="Potential",
		geoComponentUnit[result,comp,"Joules"/"Kilograms"],
...
interv[tknts_, time_, nspl_]:=Module[{},
	If[time<tknts[[4]]||time>tknts[[nspl+1]],Return[$Failed]];
	First@FirstPosition[tknts,_?(time<=#&)]
];
b0[tknts_,i_]:=3.0/(tknts[[i+3]]-tknts[[i]]);
b1[tknts_,i_]:=-3.0/(tknts[[i+4]]-tknts[[i+1]]);
...
gSpline[m_,n_][t_?NumericQ]:=Module[{nleft=interv[tknts, t, nsplines],spl,k=klist[0,m,n]},
	spl=bspline[tknts, t, nsplines, nleft];
	Sum[spl [[j]]*gt[[j+nleft-4,k]],{j,1,4}]
];
hSpline[0,n_][t_]:=0;
hSpline[m_,n_][t_?NumericQ]:=Module[{nleft=interv[tknts, t, nsplines],spl,k=klist[1,m,n]},
	spl=bspline[tknts, t, nsplines, nleft];
	Sum[spl [[j]]*gt[[j+nleft-4,k]],{j,1,4}]
];
...
iPoleFinder[date_,model_]:=Module[{yeardiff, baseyear=pickYear[model,DateValue[date,"Year"]],
	g0t,h1t,g1t,phi,lambda},
...
(*dip pole codes*)
eastcomp[long_?NumericQ,lat_?NumericQ,date_,Automatic]:=QuantityMagnitude[
	System`GeomagneticModelData[{lat, long},date,"EastComponent","Method"->{"Time"->date}]];
northcomp[long_?NumericQ,lat_?NumericQ,date_,Automatic]:=QuantityMagnitude[
	System`GeomagneticModelData[{lat, long},date,"NorthComponent","Method"->{"Time"->date}]];
eastcomp[long_?NumericQ,lat_?NumericQ,date_,alt_]:=QuantityMagnitude[
	System`GeomagneticModelData[{lat, long,alt},date,"EastComponent","Method"->{"Time"->date}]];
northcomp[long_?NumericQ,lat_?NumericQ,date_,alt_]:=QuantityMagnitude[
	System`GeomagneticModelData[{lat, long,alt},date,"NorthComponent","Method"->{"Time"->date}]];
...
iPoleFinder[comp_?(MemberQ[$dippoles,#]&),date_,model_,opt:OptionsPattern[]]:=Module[
	{north0,long,south0,lat,sol,alt=OptionValue["Altitude"],baseyear=pickYear[model,DateValue[date,"Year"]]},
	If[baseyear===$Failed,Return[$Failed]];
	north0={{long,-70,-180,180},{lat,70,0,90-$MachineEpsilon}};
	south0={{long,70,-180,180},{lat,-70,-90+$MachineEpsilon,0}};
	sol=With[{limits=If[StringMatchQ[comp,"North"~~__],north0,south0]},
		Quiet[FindRoot[{eastcomp[long,lat,date,alt],northcomp[long,lat,date,alt]},limits],
			{FindRoot::nlnum, GeoElevationData::timeout, GeoElevationData::data}]
	];
	sol={lat,long}/.sol;
...
iGeomagneticModelCompute[{lat_?NumericQ,long_?NumericQ,elev_},Array,date_,model_,opt:OptionsPattern[]]:=Module[{pot,field,
	baseyear=pickYear[model,DateValue[date,"Year"]],yeardiff},
...
iGeomagneticModelCompute[{lat_?NumericQ,long_?NumericQ,elev_},comp_String,date_DateObject,model_,opt:OptionsPattern[]]:=Module[{field,
	baseyear=pickYear[model,DateValue[date,"Year"]],yeardiff,unitlessQ=OptionValue["Unitless"]},
	If[baseyear===$Failed,Return[Missing["NotAvailable"]]];
	If[MatchQ[baseyear,_List],
		yeardiff=If[MatchQ["GUFM1",baseyear[[2]]],baseyear[[1]],First[DateDifference[{baseyear[[1]],1},date,"Year"]]];
		baseyear=baseyear[[2]],
...
iGeomagneticModelCompute[{{lat1_,long1_},{lat2_,long2_},elevation_},comp_?(StringQ[#]||MatchQ[#,All]&),date_DateObject,model_,opt:OptionsPattern[]]:=Module[
	{lam,zoom,points,rows,columns,elevationmeters=elevation,
		lat,input,pot,baseyear=pickYear[model,DateValue[date,"Year"]],yeardiff,result,quantity},
	zoom=OptionValue[GeoZoomLevel];
