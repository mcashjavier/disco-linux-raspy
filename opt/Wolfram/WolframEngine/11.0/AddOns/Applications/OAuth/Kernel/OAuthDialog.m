...
OAuthClient`tokenOAuthDialog[first_, name_String, icon_: OAuthClient`defaultServiceConnectIcon]:=OAuthClient`tokenOAuthDialog[first, {name,None}, icon]/; $CloudEvaluation

OAuthClient`tokenOAuthDialog[{url_, redirectFun_, temptoken_}, {name_, uuid_}, icon_: OAuthClient`defaultServiceConnectIcon] :=
...
Block[{res},
	$Clicked=True;
	  ...
	  If[TrueQ[$OAuthDialogSaveQ]&&StringQ[uuid],
	  	OAuthClient`saveOAuthConnection[ServiceObject[name,"ID"->uuid],Automatic]
	  ];
	]/;!$Clicked

enddialog[___]:=NotebookClose[EvaluationNotebook[]]

OAuthClient`tokenOAuthDialog[url_, name_,icon_:OAuthClient`defaultServiceConnectIcon] :=
...
KeyDialog[name_,icon_:OAuthClient`defaultServiceConnectIcon] :=
	Block[{nb, value = Null, done = False, key = "", header, footer,ifield, saveQ=OAuthClient`$SaveConnectionDefault},
		header = Panel[Grid[
			{{Spacer[20], If[icon==="","",ImageResize[icon, {Automatic, 32}]], wolframconnector}}],
			Appearance -> headerbg, Alignment -> {Left, Center}, ImageSize -> {500, 50}];
...
choicebtns[dyn : Dynamic[fieldVals_], labels : {okLbl_, cancelLbl_}, function_Function] := Grid[{{
    ...
    button[Style[okLbl, White], function, Appearance ->
      {
...
backgroundStripeFooter[content_, bkgrndImgID_: footerbg] := Panel[content,
	ImageSize -> {550, Full},
	Appearance -> {"Default" -> bkgrndImgID},
	Alignment -> {Left, Center},
	FrameMargins -> {{20,Automatic},{Automatic,Automatic}},
...
footer[dyn : Dynamic[fieldVals_], btnLabel_, btnfunction_Function, checkbox : True] := (
  ...
  labeledcheckbox[Dynamic[$OAuthDialogSaveQ], TextCell[tr["SaveConnectionTxt"], FontSize -> 10]]]
 )
...
instructions[listOfInstr_List, gridOpts : OptionsPattern[]] := Module[{},
...
MultipleKeyDialog[name_, fieldsP_, accountURL_, tosURL_, icon_: OAuthClient`defaultServiceConnectIcon] := Module[{values, serviceName = wolframconnector, includeCheckbox = True},
 (
  	$TermsAndConditionsAccepted = False;
...
OtherDialog[name_, tosText_, icon_: OAuthClient`defaultServiceConnectIcon] := Module[{values, serviceName = wolframconnector, includeCheckbox = True},
 (
 	$TermsAndConditionsAccepted = False;
