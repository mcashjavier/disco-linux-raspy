Begin["FlickrOAuth`"] (* Begin Private Context *) 
...
flickrimport[$Failed]:=Throw[$Failed]
flickrimport[json_]:=With[{res = ImportString[json, "RawJSON"]}, 
 If[res["stat"] =!= "fail", res, 
  Message[ServiceExecute::apierr, res["message"]];
  Throw[$Failed]]]
flickrnoimport[data_]:=With[{res = data},
	FromCharacterCode[res,"UTF8"]]
...
flickrcookeddata[prop_,id_,rules___Rule]:=flickrcookeddata[prop,id,{rules}]
...
flickrcookeddata["UserData", id_, args_?OptionQ] := Block[{params={"format"->"json","nojsoncallback"->"1"},rawdata,userdata,invalidParameters,user,
	...
	(
		loginInfo = ImportString[(OAuthClient`rawoauthdata[id,"RawTestLogin",{"format" -> "json", "nojsoncallback" -> "1"}]),"RawJSON"];
		AppendTo[params,"user_id"->loginInfo["user"]["id"]]
	)];
	
	rawdata = flickrimport@OAuthClient`rawoauthdata[id,"RawPeopleGetInfo",params];
	
	userData = Lookup[rawdata,"person"];

	userData = Replace[userData, {asoc_Association :> KeyMap[
		Replace[#, {"nsid" :> "NSID","ispro":>"IsPro","iconfarm":>"IconFarm","iconserver":>"IconServer","realname":>"RealName",
			"firstdatetaken"->"FirstDateTaken","firstdate"->"FirstDate", x_String :> camelCase[x]}]&, asoc]}, {0,Infinity}];
	userData = Replace[userData, {asoc_Association :> (If[MatchQ[#, Null|""], Missing["NotAvailable"], #]&/@asoc)}, {0,Infinity}];
	userData = Replace[userData, (<|"Content" -> x_|>) :> x, -2];
	AssociateTo[userData["Photos"],
		{"FirstDateTaken" -> Quiet[Check[DateObject[userData["Photos"]["FirstDateTaken"]],Missing["NotAvailable"]]],
		"FirstDate" -> Quiet[Check[FromUnixTime[ToExpression[userData["Photos"]["FirstDate"]]], Missing["NotAvailable"]]]}];
	userData
]
...
getallparameters[str_]:=DeleteCases[Flatten[{"Parameters","PathParameters","BodyData","MultipartData"}/.flickrdata[str]],
...
getUserId[userParam_,id_] := Module[{userinfo,userid=""},
	Switch[userParam,
			_String,
...
{FlickrOAuth`Private`flickrdata,FlickrOAuth`Private`flickrcookeddata,FlickrOAuth`Private`flickrsendmessage}
