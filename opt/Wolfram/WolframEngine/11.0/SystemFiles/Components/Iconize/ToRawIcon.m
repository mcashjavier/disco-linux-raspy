...
toicon[expr_NotebookObject | expr_Notebook, sz_, bg_] := Module[{pre, isz, pad, temp, img},
	...
	If[simpNotebook === $Failed, {"Notebook", $Failed}, 
		pre = ImageCrop[Rasterize[simpNotebook, ImageResolution -> $ImageResolution]];
...
toicon[type_ExportForm, sz_, bg_]:= Module[{deployPad, documentIcon, format,imageSize, stringLength, text},
	...
	text = Rasterize[exportedFileStyle[format], Background -> None, ImageResolution -> 500, ImageSize -> imageSize, ImageResolution -> $ImageResolution];
...
toicon[{x__}, sz_, bg_] := ({"DynamicObjectList", makeThumbnail[ImageCollage[Rasterize[#, ImageResolution -> $ImageResolution]&/@{x}, Background -> hexToRGB[bg]], sz, bg]}) /; dynamicObjectListQ[x]
