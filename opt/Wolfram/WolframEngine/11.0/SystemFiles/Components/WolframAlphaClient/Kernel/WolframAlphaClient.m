...
DynamicModule[{emphasizeErrors = False},
	...
	(* inner bits *)
		...
		(* display for when the parse is a formatted generalized entity *)
			$LinguisticAssistantEntityDisplay && generalizedEntityLabeledBoxesQ[boxes],
			Grid[{Flatten @ {
...
generalizedEntityLabeledBoxesQ[boxes_] := MatchQ[boxes, InterpretationBox[DynamicModuleBox[_, TemplateBox[{_, _, _, _, _}, "GeneralizedEntityToggleLabeled", ___]], _Entity, ___]]

(* Add a label to generalized entities *)
transformControlEqualBoxes[InterpretationBox[DynamicModuleBox[vars_, TemplateBox[{base_, props_, d_, tip_}, "GeneralizedEntityToggle"]], ent_Entity, opts___], query_] :=
...
(* When connected to earlier front ends, we'll need to include an explicit DisplayFunction *)
...
linguisticBottomInputFieldGeneralizedEntity[Dynamic[boxes_], Dynamic[open_], editQ: True, editTooltip_] :=
	ReplaceAll[RawBoxes[boxes],
		TemplateBox[{base_, props_, d_, tip_, label_}, "GeneralizedEntityToggleLabeled", opts___] :>
			Block[{activeLabel},
				activeLabel = ToBoxes @
					Pane[#, ImageMargins -> {{0,3},{0,0}}, BaselinePosition -> Baseline]& @ 
					Tooltip[#, editTooltip]& @
				...
				TemplateBox[{base, props, d, tip, activeLabel}, "GeneralizedEntityToggleLabeled", opts] /; True
			]
	]
...
doNewFastParse[query_, contextinfo_, Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_], Dynamic[querystate_]] :=
	Block[{queryurl, querylocalfile, queryobj, rawparse, parse, newassumptions = {}, $AlphaQueryMMode = "inline", QuantityUnits`Private`$WolframAlphaInputFlag = True,
			online, allowed, now, rememberState, newquerystate, $MessageList = {}, asyncTaskTimeout, asyncTaskDone = False, rawboxes},
		...
		If[online === False || allowed === False || MemberQ[{"error: mparse timedout", "error: query timedout", "error: no parse", "error: messages"}, query],
			...
			TimeConstrained[
				...
				queryobj = URLSaveAsynchronous[queryurl, querylocalfile, If[#2 === "data", asyncTaskDone = True]&, BinaryFormat->True, "Progress"->False];
		...
		If[parse === None,
			...
			boxes = TimeConstrained[
				...
				transformControlEqualBoxes[AlphaQueryMakeBoxes @@ parse, query],
...
doNewFastParse[query_, contextinfo_, Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_], Dynamic[querystate_]] :=
	Block[{rawparse, parse, newassumptions, $AlphaQueryMMode = "inline", QuantityUnits`Private`$WolframAlphaInputFlag = True},
		rawparse = TimeConstrained[WolframAlpha[query, "RawMathematicaParse", InputAssumptions -> assumptions], $LinguisticAssistantTimeout * 0.6, $TimedOut];
		Switch[rawparse,
			{__Rule},
			parse = Replace["Parse" /. rawparse, {Except[_String] -> None}];
			newassumptions = Replace["Assumptions" /. rawparse, {Except[_List] -> {}}],
		...
		If[parse === None && newassumptions === {},
			rawparse = TimeConstrained[WolframAlpha[query, "PrimaryMathematicaString", InputAssumptions -> assumptions], $LinguisticAssistantTimeout * 0.4, $TimedOut];
			parse = Replace[rawparse, {Except[_String] -> None}];
			newassumptions = {}
		];
		
		If[parse === None, boxes = "None", boxes = transformControlEqualBoxes[ToExpression[parse, InputForm, AlphaQueryMakeBoxes], query]];
		open = If[TrueQ[$LinguisticAssistantCompact], {1,2}, {1,2}];
		allassumptions = DeleteCases[newassumptions, {___, "type" -> (Alternatives @@ $FormulaAssumptionTypes), ___}];
		assumptions = Select[assumptions, !FreeQ[allassumptions, #]&]
	]
