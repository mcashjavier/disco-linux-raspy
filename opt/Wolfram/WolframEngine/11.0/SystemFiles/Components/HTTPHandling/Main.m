...
(*this is for tracking the broker process*)
$MQTTBroker = Null;
	...
	Block[{stderr,exitcode},
		With[
			...
			(
				...
				If[ProcessStatus[$MQTTBroker]=!="Running",
					(*THEN*)
					(*it's stopped for some reason, issue a message and don't try to publish*)
					(
						stderr = ReadString @ ProcessConnection[$MQTTBroker, "StandardError"];
						exitcode = ProcessInformation[$MQTTBroker, "ExitCode"];
...
(ws:WebServer[_?AssociationQ])["Running"] := And[
	ProcessStatus[$MQTTBroker]==="Running",
...
(ws:WebServer[_?AssociationQ])["RegisterHandler", expr_] := (
	AppendTo[$WebServers[ws["Port"]], "Handler" :> expr];
	ws
)
...
(ws:WebServer[_?AssociationQ])["StartConnection"] := Module[{client},
	(
		client = CreateClient[
			"localhost",
			1883,
			"ClientDisconnectedFunction":>(ConnectClient[#2]&)
		];
...
(ws:WebServer[_?AssociationQ])["StartServer", force_:False, startserver_:True] := Block[
	{serverinfo, port, serverProcess, connection},
	port = ws["Port"];
	If[ws["Running"] && !force,
	...
	If[Not[MemberQ[{connection,serverProcess,$MQTTBroker},$Failed]],
		...
		(
			serverinfo["Server"] = serverProcess;
			serverinfo["Connection"] = connection;
			$WebServers[port] = serverinfo;
			ws
		),
		(*ELSE*)
