...
Begin["MixpanelAPI`"] (* Begin Private Context *)
...
mixpaneldata[]={
		...
		"URLFetchFun"		:> (Module[{parameters,buffer,apisecret,sig,expire},
						    	parameters = (ToString[First@#]->ToString[Last@#])&/@Lookup[{##2},"Parameters",{}]; apisecret=Lookup[parameters,"apisecret",Throw[$Failed]];
						    	expire = ToString[FromDigits[RealDigits[AbsoluteTime[TimeZone->0]-AbsoluteTime[DateList[{1970, 1, 1, -1, 0, 0}]]][[1,1;;10]]]];
						    	buffer = Join[parameters,{"format"->"json", "expire"->expire}];
						    	sig = IntegerString[Hash[StringJoin[Replace[Sort[buffer], Rule[f_, l_] :> f <> "=" <> l, {1}], apisecret], "MD5"], 16, 32];
						    	AppendTo[buffer, "sig" -> sig];
						    	URLFetch[#1, {"StatusCode","Content"}, "Parameters" -> buffer, "Headers" -> {}]]&)
        	,
		"ClientInfo"		:> OAuthDialogDump`Private`MultipleKeyDialog["Mixpanel",{"API Key"->"api_key","API Secret"->{"apisecret",FieldMasked->True}},"https://mixpanel.com/login/","https://mixpanel.com/terms/"],
...
(* Auxiliar functions*)
mixpanelimportdata[rawdata_]:=Module[{},
	If[rawdata=!="",ImportString[Last@rawdata,"RawJSON"],Missing["NotAvailable"]]
	]
...
mixpaneldata["RawAnnotationUpdate"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/annotations/update",
        "HTTPSMethod"		-> "POST",
...
mixpaneldata["RawEventsNames"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/events/names/",
        "HTTPSMethod"		-> "GET",
        "Parameters"		-> {"type","limit"},
        "RequiredParameters"-> {"type"},
        "ResultsFunction"	-> mixpanelimportdata
...
mixpaneldata["RawEventProperties"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/events/properties/",
        "HTTPSMethod"		-> "GET",
...
mixpaneldata["RawEventPropertiesTop"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/events/properties/top/",
        "HTTPSMethod"		-> "GET",
...
mixpaneldata["RawEventPropertiesValues"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/events/properties/values/",
        "HTTPSMethod"		-> "GET",
...
mixpaneldata["RawSegmentation"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/segmentation/",
        "HTTPSMethod"		-> "GET",
...
mixpaneldata["RawSegmentationNumeric"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/segmentation/numeric/",
        "HTTPSMethod"		-> "GET",
...
mixpaneldata["RawSegmentationSum"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/segmentation/sum/",
        "HTTPSMethod"		-> "GET",
...
mixpaneldata["RawSegmentationAverage"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/segmentation/average/",
        "HTTPSMethod"		-> "GET",
        "Parameters"		-> {"event","from_date","to_date","on","unit","where"},
        "RequiredParameters"-> {"event","from_date","to_date","on"},
        "ResultsFunction"	-> mixpanelimportdata
...
mixpaneldata["RawRetention"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/retention/",
        "HTTPSMethod"		-> "GET",
...
mixpaneldata["RawRetentionAddiction"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/retention/addiction",
        "HTTPSMethod"		-> "GET",
...
mixpaneldata["RawEngage"]:={
        "URL"				-> "http://mixpanel.com/api/2.0/engage/",
        "HTTPSMethod"		-> "GET",
	...
	Which[
			raw[[1]] === 200,
				data = mixpanelimportdata@raw; pvalues = Replace[data["data"]["values"],{asoc_Association :> KeyMap[If[StringMatchQ[#, Alternatives["mp_" ~~ ___, "$" ~~ ___]], 
						OAuthClient`Private`camelCase[#,{"mp_","_","$"}], #] &, asoc]}];
				If[pvalues===<||>,
...
mixpanelcookeddata["TopEvents",id_, args_?OptionQ] :=  Block[{invalidParameters, raw, type, limit, data},
	...
	Which[
			raw[[1]] === 200,
				data = Replace[(mixpanelimportdata@raw)["events"], asoc_Association :> KeyMap[OAuthClient`Private`camelCase, asoc], 1];
				data[[All, "Event"]] = If[StringMatchQ[#, Alternatives["mp_" ~~ ___, "$" ~~ ___]], OAuthClient`Private`camelCase[#,{"mp_","_","$"}], #] & /@ data[[All, "Event"]];
			...
			raw = KeyClient`rawkeydata[id,"RawEventProperties", {"event" -> event, "name" -> #, "type" -> type, "interval" -> interval, "unit" -> unit, "limit" -> maxItems}];
				...
				pvalues = data["data"]["values"];
					If[pvalues===<||>,
						...
						pvaluesObj = Replace[pvalues,{asoc_Association :> KeyMap[Replace[#, svrep] &, asoc]},1];
						setvalues = Transpose@Replace[pvaluesObj,{x_Association :> KeySortBy[KeySelect[x, MemberQ[svalues, #] &],AbsoluteTime]},1];
						<|Replace[str_String :> OAuthClient`Private`camelCase[str, {"mp_", "_", "$"}]][#]->MapThread[Prepend, {Values[setvalues], ("Date" -> #) & /@ Keys[setvalues]}]|>
					]
			]&/@name;
			Dataset[datalist]
	]
...
mixpanelcookeddata["Funnel",id_, args_?OptionQ] :=  Block[{invalidParameters, dateRules, raw, errmsg, data, camelData, funnelId, deadline, window, startD, endD, var, unit},
	...
	Which[
			raw[[1]] === 200,
				...
				camelData = Replace[data, str_String?(StringMatchQ[#, Alternatives["mp_" ~~ __, "$" ~~ __]]&):> OAuthClient`Private`camelCase[str,{"mp_","_","$"}], {4}];
