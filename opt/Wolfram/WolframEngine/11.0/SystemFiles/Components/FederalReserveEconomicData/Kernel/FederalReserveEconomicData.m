Begin["FederalReserveEconomicData`"] (* Begin Private Context *)
...
(*fredcookeddata[req_, id_]:=fredcookeddata[req, id, {}]*)
fredcookeddata[prop_,id_,rules___Rule]:=fredcookeddata[prop,id,{rules}]

fredcookeddata["SeriesSearch", id_, args_?OptionQ] := Module[{rawdata, invalidParameters, params={"file_type"->"json"}, filter, 
	...
	(
		(	
			...
			rawdata = KeyClient`rawkeydata[id,"RawSeriesSearch",params];
			data = ImportString[rawdata[[2]],"RawJSON"];
			
			If[rawdata[[1]]!=200,
			(
				Message[ServiceExecute::serrormsg, Lookup[data,"error_message"]];
				Throw[$Failed]
			)];
	...
	(
		...
		rawdata = KeyClient`rawkeydata[id,"RawSeriesSearch",params];
		data = ImportString[Last@rawdata,"RawJSON"];
		
		If[(First@rawdata)!=200,
		(
			Message[ServiceExecute::serrormsg, Lookup[data,"error_message"]];
			Throw[$Failed]
		)];
...
fredcookeddata["SeriesData", id_, args_?OptionQ] := Module[{rawdata, invalidParameters, params={"file_type"->"json"}, ids, dateRange, 
	...
	(
		...
		rawdata = KeyClient`rawkeydata[id,"RawSeriesObservations",params];
		data = ImportString[Last@rawdata,"RawJSON"];
...
formatresults[rawdata_] := ToString[rawdata[[2]],CharacterEncoding->"UTF-8"]
