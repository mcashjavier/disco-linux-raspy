...
General::notloaded = "adapter is not loaded successfully";
General::osnotsupp = "`1` is not supported on `2`.";
General::nofile = "`1` does not exist.";
General::nodir = "`1` does not exist.";
General::noFromAddress = "There is no From address.";
...
$library = FileNameJoin[{FileNameTake[$packageFile, {1,-2}], "LibraryResources", $SystemID, $libraryFileName}];
$libraryGMime = FileNameJoin[{FileNameTake[$packageFile, {1,-2}], "LibraryResources", $SystemID, Switch[$OperatingSystem, "Windows", "gmime.dll", "MacOSX", "TODO", "Unix", "libgmime-2.6.so.0"]}];

$asyncObj = Null;
$adapterLoaded = False;

loadAdapter[]:= If[$libraryFileName === $Failed,
	Message[loadAdapter::osnotsupp, "MIMETools", $OperatingSystem]
	,
Module[{},
If[!$adapterLoaded,
...
Lookup[
	SelectFirst[
		MIMEMessageRead[obj, "DecodedRawBody"]
		, StringMatchQ[ToLowerCase[Lookup[#, "ContentType", ""]], "text/plain"] &
		, <||>
	]
	, "Contents"
	, None
...
If[$libraryFileName === $Failed,
	Message[MIMEMessageCreate::osnotsupp, "MIMETools", $OperatingSystem]
	,
	Module[{optstemp, inputlist,fromArg},
		If[$adapterLoaded == True||loadAdapter[]=!=$Failed,
			...
			If[fromArg===None, Message[MIMEMessageCreate::noFromAddress]];
			lfMIMEStringCreate[Sequence@@Flatten[inputlist]]
			,
			Message[loadAdapter::notloaded];
			Return@$Failed;			
		]
	]
];
...
(*Options[MIMEMessageUnquotedBody] = {Bound -> ""};

MIMEMessageUnquotedBody[obj_,OptionsPattern[]]:=
Module[{body, lines, mainBody, main, unqoatedlines, unqoatedparts,boundary}, 
 body = MIMEMessageRead[obj, "Body"];
 boundary = OptionValue[Bound];
 If[StringMatchQ[boundary, "---"~~___~~"Original Message"~~___~~"---"~~___],
	List@StringSplit[body, boundary][[1]][[1]]
	,
	lines = StringSplit[body[[1]], "\n>"];
	If[StringMatchQ[lines[[1]], ">" ~~ ___] || StringMatchQ[lines[[1]], "----" ~~ ___] === True,
		mainBody = 0;
		main = "", main = lines[[1]];
		mainBody = 1;
	];
	filter[x_String] := StringMatchQ[x, ___ ~~ "\n" ~~ ___ ~~ "\n" ~~ ___];
	If[mainBody === 1, 
		unqoatedlines = Select[lines[[2 ;;]], filter];
		unqoatedparts = StringExtract[unqoatedlines, "\n" -> 2 ;;]
		,
		unqoatedlines = Select[lines, filter]; 
		unqoatedparts = StringExtract[unqoatedlines, "\n" -> 2 ;;];];
		DeleteCases[Join[{main}, Catenate[unqoatedparts]], ""]
	]
 ]*)
...
Module[{optstemp},
	If[$adapterLoaded == True||loadAdapter[]=!=$Failed,
	optstemp = ({#[[1]], Switch[Head@#[[2]], List, Length@#[[2]], String, 1], 
	  If[#[[1]] === "Attachments" || #[[1]] === "Inline", 
		Switch[Head@#[[2]], List, Map[ImportExport`FileUtilities`GetFilePath, #[[2]]], String,ImportExport`FileUtilities`GetFilePath[#[[2]]]]
		,
		Sequence[#[[2]]]
		]}) & /@ {opts};
	lfMIMEStringCreate[Sequence@@Flatten[optstemp]]
	,
...
If[$libraryFileName === $Failed,
	Message[MIMEMessageOpen::osnotsupp, "MIMETools", $OperatingSystem]
	,
	If[$adapterLoaded == True||loadAdapter[]=!=$Failed, 
		Module[{res},
			res = lfMIMEStringOpen[StringReplace[message, RegularExpression["^\\s*"] -> ""]];
			If[Head@res === MIMEToolsException,
				Return[res]
				,
				MIMEMessage[res]
			]
	...
	Module[{filename,rawFilePath},
		If[$adapterLoaded == True||loadAdapter[]=!=$Failed
			...
			If[StringQ[rawFilePath] && FileType[rawFilePath] === File
				...
				,
				Message[MIMEMessageOpen::nofile,rawFilePath];
				Return@$Failed;
			]
			,
			Message[loadAdapter::notloaded]; 
...
Options[MIMEMessageCreate] = {"MessageContentType" -> None}
(* Read will call processOutput which determines based on the field how to process and return the data *)
MIMEMessageRead[obj_?openMessageQ, field : (	"MessageContentType" | "BodyContentType" | "Header" | "HeaderString" | 
											"BodyHeaders" | "Body" | "Attachments" | "RawHeader" | "RawBodyHeaders" | 
											"RawBody" | "RawAttachments" | "HasAttachments" | "DecodedRawBody"), headers_List:{}]:= 
If[$libraryFileName === $Failed,
	Message[MIMEMessageRead::osnotsupp, "MIMETools", $OperatingSystem]
	,
	Module[{streamID},
		If[$adapterLoaded == True||loadAdapter[]=!=$Failed, 
			streamID = obj[[1]];
			processOutput[field, lfMIMEMessageLookup[streamID,Replace[field, fieldSource]], Replace[headers, {{} -> Sequence[]}]]
			,
			Message[loadAdapter::notloaded];
			Return@$Failed;
		]
...
processOutput["BodyHeaders" | "DecodedRawBody", lookupRet_List] := wolframizeHeaders[wolframizeValues[lookupRet]]
...
processOutput["Body", lookupRet_List] := wolframizeHeaders[importObject[#] & /@ wolframizeValues[lookupRet]]
...
importAttachment[assoc_Association, "Octet"] :=
	Replace[Quiet@ImportString[Lookup[assoc, "Contents", ""]], $Failed :> CreateMIMEToolsException[39, None, {wolframizeHeaders[wolframizeValues[assoc]], "Binary"}], {0}]
...
Association[KeyValueMap[
	Switch[#1, 
		"Content-Transfer-Encoding", #1 -> Replace[ToLowerCase[#2], wolframEncodings], 
		"Content-Disposition", #1 -> Replace[ToLowerCase[#2], wolframDispositions],
		"Content-Type", #1 -> ToLowerCase[#2],
		"Type", #1 -> ToLowerCase[#2],
		"Subtype", #1 -> ToLowerCase[#2],
		_, #1 -> #2
	] &, #]] & /@ l;
...
(* Precedence *)
processHeaderField["Precedence", p_String]:= Replace[p, {"list" -> "List", "first-class" -> "FirstClass", "bulk" -> "Bulk", "junk" -> "Junk"}, {0}]
...
processHeaderField["OriginatingTimezone", date_String] := Quiet@Lookup[Options[processHeaderField["OriginatingDate", date], TimeZone], TimeZone, Missing["NotAvailable"]]
...
If[Length@l > 0,
	Quiet@Lookup[Options[processHeaderField["ServerOriginatingDate", l], TimeZone], TimeZone, Missing["NotAvailable"]]
	,
	Missing["NotAvailable"]
]
...
processHeaderField[type : (	"DeliveryChainHostnames" | "OriginatingHostname" | "OriginatingIPAddress" | 
							"OriginatingCountry" | "ServerOriginatingDate" | "ServerOriginatingTimezone")
							,str_String] := 
	processHeaderField[type, StringSplit[str, "from"]]

processHeaderField[	"FromAddress" | "ToAddressList" | "CcAddressList" | "ReplyToAddressList" | "FromName" | 
					...
					"BccAddressList" | "ReplyToAddressList"| "Precedence", expr_] := 
	expr

processHeaderField[	"DeliveryChainHostnames" | "DeliveryChainRecords" | "OriginatingHostname" | 
					"OriginatingIPAddress" | "OriginatingCountry" | "ServerOriginatingDate" | 
					"ServerOriginatingTimezone" | "FromAddress" | "FromName", expr_Missing] := 
	Missing["NotAvailable"]

processHeaderField[	"ToAddressList" | "CcAddressList" | "ToNameList" | "CcNameList" | "BccNameList" | "ReplyToNameList" |
					"BccAddressList" | "BccNameList" | "ReplyToAddressList" | "ReplyToAddressList" , expr_Missing] := 
	{}
...
(* Specifies the order of the fields returned by MIMEMessageRead[obj, "Header"] *)
(* 
order = {
	"MIMEMessageClass",
	"ReturnPath",
...
(*MIMEFileLookup[file_String,field_String]:=
If[$libraryFileName === $Failed,
	Message[MIMEFileLookup::osnotsupp, "MIMETools", $OperatingSystem]
	,
	If[$adapterLoaded == True||loadAdapter[]=!=$Failed,		 
		lfMIMEMessageLookup[file,field]
		,
		Message[loadAdapter::notloaded]; 
		Return@$Failed;
	]
...
If[$libraryFileName === $Failed,
	Message[MIMEMessageClose::osnotsupp, "MIMETools", $OperatingSystem]
	,
	Module[{streamID},
		If[$adapterLoaded == True||loadAdapter[]=!=$Failed, 
			streamID = obj[[1]];
			lfMIMEMessageClose[streamID];
			Return[Null];
			,
			Message[loadAdapter::notloaded]; 
			Return@$Failed;
...
If[$libraryFileName === $Failed,
	Message[MIMEMessageParse::osnotsupp, "MIMETools", $OperatingSystem]
	,
	Module[{obj, retVal},
		If[$adapterLoaded == True||loadAdapter[]=!=$Failed,
...
(* ------------------------------------------------------------------------- *)
(* ------------------------------------------------------------------------- *)
(* Turned off for now, changes to the CSOURCE makes this not work *)


(* MIMEAttachmentSave[obj_,attDir_]:=
If[$libraryFileName === $Failed,
	Message[MIMEAttachmentSave::osnotsupp, "MIMETools", $OperatingSystem]
	,
	Module[{streamID},
		If[$adapterLoaded == True||loadAdapter[]=!=$Failed, 
			streamID = obj[[1]];
			check whether attDir exists, if not, fail gracefully
			If[FileNames[attDir] === {},
				Message[MIMEAttachmentSave::nodir, attDir];
				Return@$Failed;
				,
				lfMIMEMessageLookup[streamID, "Attachments",attDir<>"/"]
			]
			,
			Message[loadAdapter::notloaded];
			Return@$Failed;
...
getOpenMIMEMessage:=
If[$libraryFileName === $Failed,
	Message[getOpenMIMEMessage::osnotsupp, "MIMETools", $OperatingSystem]
	,
	If[$adapterLoaded == True||loadAdapter[]=!=$Failed, 
		Map[MIMEMessage,lfGetOpenMIMEStream["MIME"]]
		,
		Message[loadAdapter::notloaded]; 
		Return@$Failed;
	]
...
(*	messageID = Replace[ MIMEMessageGetElement[msgObj, "MessageID"], exc_MIMEToolsException -> Missing["NotAvailable"]]; 
	authorName = Replace[MIMEMessageGetElement[msgObj, "FromName"], exc_MIMEToolsException -> Missing["NotAvailable"]];
	authorAddress = Replace[MIMEMessageGetElement[msgObj, "FromAddress"], exc_MIMEToolsException -> Missing["NotAvailable"]];
	author = Replace[MIMEMessageGetElement[msgObj, "From"], exc_MIMEToolsException -> Missing["NotAvailable"]];
	date = Replace[MIMEMessageGetElement[msgObj, "OriginatingDate"], exc_MIMEToolsException -> Missing["NotAvailable"]]; *)
...
MIMEBodyParseInternal[body_String, 
	...
	If[type === "Attribution",
		...
		Map[Function[arg,
			...
			Switch[type, "NewBodyContent", selectedBlock,
				"QuotedContent", quotedBlock,
				"AllQuotedContent", allQuotedBlock,
				"PairedContent", Sort[Union[selectedBlock, quotedBlock], #1[[1]][[1]] < #2[[1]][[1]] &],
				All, messageBlock]
			]
	];
...
Module[
	...
	Switch[type,
		"NewBodyContent",
		(* Sort[Select[totalElement, #[[2]]["Level"]==0&], #1[[2]]["StartEndLineNumbers"][[1]] < #2[[2]]["StartEndLineNumbers"][[1]]&], *)
		unquotedElement,
		"QuotedContent",
		quotedElement,		
		"Attribution",
