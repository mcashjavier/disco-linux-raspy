...
styledtxt[txt_, "wolframid", colr_] := styledtxt[txt, "basefont", FontSize -> $WolframIDFontSize, LineBreakWithin -> False, FontColor -> colr];
...
errmssgTR[errorCode_] := PaneSelector[
	{
		"1500v11" -> Dynamic[FrontEndResource["WolframCloudLoginErrors", "1500v11"], DestroyAfterEvaluation -> True],
		"1600v11" -> Dynamic[FrontEndResource["WolframCloudLoginErrors", "1600v11"], DestroyAfterEvaluation -> True],
		"Default" -> Dynamic[FrontEndResource["WolframCloudLoginErrors", errorCode], DestroyAfterEvaluation -> True]
	},
	Dynamic[
		Which[
			errorCode == 1500 && BoxForm`sufficientVersionQ[11], "1500v11",
			errorCode == 1600 && BoxForm`sufficientVersionQ[11], "1600v11",
			True, "Default"
		]
	],
	Alignment -> {Center, Center}
...
errmssg[errorCode_, isWCUILogin_, stylesuffix_String, opts:OptionsPattern[]] := PaneSelector[
					{
						True -> errmssg[errorCode, stylesuffix],
						False -> ""
					},
...
errmssg[errorCode_, stylesuffix_String] := styledtxt[errmssgTR[errorCode], "errorIn"<>stylesuffix];
...
createWolframID := Framed[
	Style[
		Row[
			{
				styledtxt[tr["CloudAccountQuery"], "wolframid", "leadin"], " ", button[mouseovertxt[tr["JoinNowButtonLabel"], "wolframidlink"], "hyperlink", "MouseClicked" :> FE`hyperlinkCoded["http://www.wolfram.com/mathematica-cloud-access", "source=cloudlogindialog"]]
			}
		],
...
errorTitleColumn[errorCode_] := Column[
	{
		errorPanel[errmssg[errorCode, "ErrorPanel"]],
		titleBanner
	},
...
titlePlusErrorPanel[errorCode_, isWCUILogin_] := PaneSelector[
	{
		False -> titleBanner,
		True -> errorTitleColumn[errorCode]
	},
	Dynamic[isWCUILogin && errorCode != 0],
	ImageSize -> {Full, Automatic}
]
...
loginDialog[username_String] := With[{boxid = "username", pwdboxid =" passwd", cloudLoginTR = $CloudLoginTextResource},
	Block[
		{
			$loginCredentials
		},
				...
				DialogInput[
					ExpressionCell[
						Framed[
							Column[
								{
									titlePlusErrorPanel[CloudObject`Private`errorCode, CloudObject`Private`isWCUILogin](*titleBanner*),
									spacr[34],
