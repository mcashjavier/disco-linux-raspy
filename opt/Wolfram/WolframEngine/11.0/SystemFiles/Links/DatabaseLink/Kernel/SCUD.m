...
invalidTableNameQ[SQLTable[name_]]:=invalidTableNameQ[name]

invalidTableNameQ[name_String]:=Module[{},StringContainsQ[name," "]]
 
checkDerbyColumnType[conn_SQLConnection,col:(_SQLColumn | {__SQLColumn})] := Module[{},
	 If[
...
SQLInsert[conn_SQLConnection,
   ...
   If[checkMSAccessColumnValues[conn,values],Message[SQLInsert::unsupported];$Failed, 
    vals = If[MatchQ[values, {__List}],
        StringTemplate["(`1`)"][StringRiffle[Table["?", Range@Length@First@values], ","]],
        StringTemplate["(`1`)"][StringRiffle[Table["?", Range@Length@values], ","]]
    ];
...
SQLSelect[conn_SQLConnection,
    ...
    If[invalidTableNameQ[table],Message[SQLTable::tablename,If[Head[table]===SQLTable,First@table,table]], tbls=If[MatchQ[Flatten[{table}], {__SQLTable}], 
...
SQLDelete[ conn_SQLConnection,
           ...
           condition_, 
           opts:OptionsPattern[]] := Module[
    { useOpts, timeout, tbl, sql }, 

    useOpts = Join[canonicalOptions[Flatten[{opts}]], Options[SQLDelete]];
    timeout = Lookup[useOpts, "Timeout"];
    
  If[invalidTableNameQ[table],Message[SQLTable::tablename,If[Head[table]===SQLTable,First@table,table]], tbl = If[StringQ[table], SQLTable[table], table];
