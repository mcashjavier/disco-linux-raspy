...
standardizeContentMetadata[id_, info_]:=With[{rtype=Lookup[info,"ResourceType"]},
	If[StringQ[rtype],
...
autoloadResource[info_]:=With[{id=Lookup[info, "UUID",Throw[$Failed]]},
	If[MemberQ[$localResources,id],
		System`ResourceObject[id],
		saveresourceobject[standardizeContentMetadata[id, KeyDrop[info,"Autoload"]]]
	]
...
saveresourceObject[id_String, as_]:=saveresourceobject[resourceInfo[id]]/;MemberQ[$loadedResources,id]
...
saveresourceobject[info_]:=Block[{rtype=Lookup[info,"ResourceType"],id},
	id=If[StringQ[rtype],
		loadResourceType[rtype];
