...
EmbedCode`Common`iEmbedCode["java", apiFunc_APIFunction, url_, opts:OptionsPattern[]] :=
    Module[{sig, funcName, argTypes, retType, paramInfo, returnType, finalArgSpec, argTypeString, code, needsMultipart},
        ...
        (* Note the replacement of https with http. This is because the wolframcloud.com SSL certificate is not trusted by default in the JVM.
           This is a GoDaddy-signed certificate, and it's crazy that this authority is not in the standard JVM truststore; see
           http://stackoverflow.com/questions/18746565/godaddy-ssl-cert-not-working-with-java for more details. User who want to make
           API calls via https can add our certificate to their JVM truststore manually.
        *)
        code = header[StringReplace[url, "https" -> "http"], argTypeString, returnType, funcName,
